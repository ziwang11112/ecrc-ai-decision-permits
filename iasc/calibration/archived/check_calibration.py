...
def main():
    ...
    files += [SOURCE / "e1/routing.py", SOURCE / "e1/config.py", SOURCE / "e1/models.py", SOURCE / "run_e1.py", Path(__file__)]
    ...
    pd.DataFrame(selected_rows).to_csv(HERE / "calibration_selected_audit.csv", index=False)
    ...
    pd.DataFrame(grid_rows).to_csv(HERE / "calibration_reconstructed_grids.csv", index=False)
    pd.DataFrame(summaries).to_csv(HERE / "calibration_grid_summary.csv", index=False)
