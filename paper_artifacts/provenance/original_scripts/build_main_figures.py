"""Current paper figures: specified lifecycle and complete saved S9 outcomes.
...
def fig2(out):
    ...
    save(f,out,2)
def fig3(out):
    src=P/'supplement/current_result_index/figures/data/fig3_plot_data.csv'
    ...
    save(f,out,3)
    (out/'fig3_SOURCE.json').write_text(json.dumps({'source':'supplement/current_result_index/figures/data/fig3_plot_data.csv','source_sha256':hashlib.sha256(src.read_bytes()).hexdigest(),'rows':36,'filter':'none','observations_per_cell':6,'values_changed':False,'phase_labels':{s:{p:l.replace('\n',' ') for p,l in ph} for s,t,ph in panels}},indent=2)+'\n')

def fig4(out):
    src=P/'supplement/S9_application/S9_cells.csv'
    rows=list(csv.DictReader(src.open(encoding='utf-8')))
    ...
    save(f,out,4)
    (out/'fig4_SOURCE.json').write_text(json.dumps({'source':'supplement/S9_application/S9_cells.csv','source_sha256':hashlib.sha256(src.read_bytes()).hexdigest(),
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--out',type=Path,default=HERE/'exports');a=p.parse_args();a.out.mkdir(parents=True,exist_ok=True)
